...
build do
  ...
  gem "install --local #{ENV['FLUENTD_COMMANDER_SOURCE']}/fluentd_agent*.gem", :cwd => ENV['FLUENTD_COMMANDER_SOURCE']

  gem "build #{ENV['FLUENTD_STACKDRIVER_SOURCE']}/fluentd-stackdriver-plugin.gemspec", :cwd => ENV['FLUENTD_STACKDRIVER_SOURCE']
  gem "install --local #{ENV['FLUENTD_STACKDRIVER_SOURCE']}/fluentd-stackdriver-plugin*.gem", :cwd => ENV['FLUENTD_STACKDRIVER_SOURCE']
end
